...
1) when the total submission is reaching its limit, 
send an alert email to the administrator.
2) when the submission limit has been reached, close the webform.

One more thing we need to keep in mind is that the webfom modules 
reuse the specific webform and store the submission data in one 
single database table, so we have to figure out the specific 
submission data respect to each particular node.
